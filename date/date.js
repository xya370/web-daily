...
(function(root,factory){
    ...
    DatePicker.prototype={
        ...
        setDay:function(){
            ...
            var nw = parseInt(ndays.length/7)+1,idv=''
            for(var i=0;i<nw;i++){
                idv+='<div class="dp-day-line" style="width:100%;height:calc( 100% / '+nw+' )" week="'+(i+1)+'">'
                for(var j=0;j<ndays.length;j++){
                    if(parseInt(j/7)==i){
                        if(ndays[j] == _this.date){
                            idv+='<div class="dp-day-span select-dp-day"  day="'+ndays[j]+'">'+ndays[j]+'</div>'
                        }else{
                            idv+='<div class="dp-day-span"  day="'+ndays[j]+'">'+ndays[j]+'</div>'
                        }     
                    }
                }
                idv+='</div>'
